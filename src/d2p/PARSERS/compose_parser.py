...
class ComposeParser:
    ...
    def parse_from_string(self, content: str) -> OrchestrationConfig:
        ...
        return OrchestrationConfig(
            services=services,
            networks=(
                list(data.get("networks", {}).keys()) if data.get("networks") else []
            ),
    ...
    def _parse_service(self, name: str, spec: Dict[str, Any]) -> ServiceDefinition:
        ...
        for p in spec.get("ports", []):
            if isinstance(p, str):
                ...
                target: int = p["target"]
                published: Optional[int] = p.get("published")
        ...
        return ServiceDefinition(
            ...
            build_context=(
                spec.get("build", {}).get("context")
                if isinstance(spec.get("build"), dict)
                else spec.get("build")
            ),
            dockerfile_path=(
                spec.get("build", {}).get("dockerfile")
                if isinstance(spec.get("build"), dict)
                else None
            ),
