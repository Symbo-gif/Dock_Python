...
class NamespaceManager:
    ...
    def _detect_available_namespaces(self) -> None:
        ...
        if is_root:
            ...
            for ns_type in [NamespaceType.MOUNT, NamespaceType.UTS,
                           NamespaceType.IPC, NamespaceType.PID,
                           NamespaceType.NET, NamespaceType.CGROUP]:
    ...
    def get_effective_namespaces(self) -> NamespaceType:
        ...
        for ns_type in NamespaceType:
            if (self.config.namespaces & ns_type) and (ns_type in self._available_namespaces):
                effective |= ns_type
        return effective
    ...
    def unshare(self, namespaces: Optional[NamespaceType] = None) -> bool:
        ...
        """
        if not self.is_available:
            print("Warning: Namespace isolation not available, running without isolation")
            return False
