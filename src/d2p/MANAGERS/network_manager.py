...
class NetworkManager:
    ...
    def __init__(self) -> None:
        ...
        """
        self.service_ports: Dict[str, Dict[int, int]] = (
            {}
        ...
        self.service_networks: Dict[str, ServiceNetwork] = (
            {}
        )  # service_name -> network info
        ...
        self.create_network(
            NetworkConfig(
                name="d2p_default", subnet="172.28.0.0/16", gateway="172.28.0.1"
            )
        )
    ...
    def connect_service(
        self, service_name: str, network_name: str, aliases: Optional[List[str]] = None
    ) -> Optional[str]:
        """
        ...
        for container_port, host_port in service_def.ports.items():
            if host_port is None:
                ...
                if is_port_free(host_port):
                    ...
                    raise RuntimeError(
                        f"Port {host_port} is already in use, cannot start service {service_def.name}"
                    )
    ...
    def get_service_discovery_env(self, all_services: List[str]) -> Dict[str, str]:
        ...
        for name in all_services:
            ...
            host: str
            if name in self.service_networks:
                ip_addr = self.service_networks[name].ip_address
            ...
            if name in self.service_ports and self.service_ports[name]:
                ...
                port_value = self.service_ports[name][container_ports[0]]
                if port_value is not None:
                    env[f"{prefix}_PORT"] = str(port_value)
