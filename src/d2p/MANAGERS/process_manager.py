...
class ProcessManager:
    ...
    def __init__(self, service_def: ServiceDefinition, base_dir: str = "."):
        """
        ...
        self.runner = ProcessRunner(service_def.name, log_file=log_path)
    ...
    def start(self, extra_env: Optional[Dict[str, str]] = None):
        ...
        env = self.env_manager.get_merged_environment(
            self.service_def.environment, self.service_def.environment_files
        )
        if extra_env:
            env.update(extra_env)

        # 2. Prepare Volumes
        self.volume_manager.prepare_volumes(
            self.service_def.volumes, service_working_dir=self.service_def.working_dir
        )

        # 3. Resolve Command
        command = self.executor.get_full_command(
            self.service_def.entrypoint, self.service_def.cmd
        )
        ...
        # 4. Start Process
        self.runner.start(command, env=env, working_dir=self.service_def.working_dir)
