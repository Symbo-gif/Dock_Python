...
class HealthMonitor:
    ...
    def _monitor_loop(self):
        ...
        while self.running:
            ...
            for name, manager in self.managers.items():
                ...
                health.last_check = datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")
    ...
    def _should_restart(self, name: str, manager: ProcessManager, 
    ...
    def _handle_restart(self, name: str, manager: ProcessManager) -> None:
        ...
        if policy.max_retries > 0 and health.restart_count >= policy.max_retries:
            print(f"Service {name} exceeded max restart attempts ({policy.max_retries})")
            return
        ...
        try:
            ...
            health.last_restart = datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")
